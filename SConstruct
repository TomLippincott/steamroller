...
def Local(command):
    ...
    def local_method(target, source, env):
        ...
        with writer(gzip.open(target[-1].rstr(), "w")) as ofd:
            ofd.write(err)
        return None
    return Builder(action=Action(local_method, command), emitter=local_emitter)


def Wrapper(command, resources=[]):
    local_command = "/usr/bin/time --verbose " + command
    if env["GRID"] and isinstance(local_command, basestring):
        return Grid(local_command, resources)
    else:
        return Local(local_command)
...
for task in env["TASKS"]:
    ...
    for size in task.get("sizes", defaults.get("sizes", [])):
        for fold in range(1, task.get("folds", defaults.get("folds", 1)) + 1):
            ...
            for model in env["MODELS"]:
                ...
                classified.Tag("size", size)
                classified.Tag("task", task_name)
                classified.Tag("fold", fold)
                classified.Tag("model", model_name)
    ...
    #    compare = env.Compare("work/%s_compare.txt.gz" % (task_name), classified_items)
        #scores, resources = env.Evaluate("work/%s_scores.txt.gz" % (task_name), classified_items)
        #plots, resources = env.Plot("work/%s_plot.png" % (task_name), scores)
